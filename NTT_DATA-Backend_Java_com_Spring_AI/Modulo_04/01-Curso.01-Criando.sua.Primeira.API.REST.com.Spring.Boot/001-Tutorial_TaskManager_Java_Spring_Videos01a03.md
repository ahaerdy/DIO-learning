...
O primeiro vídeo é teórico: antes de escrever qualquer código Java, a aula explica os conceitos que justificam as decisões de design que vêm a seguir. Como você já documentou o conteúdo detalhadamente no seu README, aqui vai um resumo objetivo dos pontos que realmente importam para entender o código das próximas partes.
...
### 1.2. Os quatro verbos HTTP que você vai usar no projeto
...
- **`infrastructure`** — tudo que é "detalhe técnico": banco de dados, APIs externas, e, como você vai ver, a implementação do repositório em memória.

> **Por que separar em pacotes assim?**
> Essa separação existe para que as regras de negócio (`domain`) não fiquem "presas" a uma tecnologia específica. Se um dia você trocar de banco de dados, ou parar de usar Spring, o pacote `domain` praticamente não muda — só o que está em `infrastructure`. Pense em `domain` como as regras do jogo, e `infrastructure` como o material usado para jogar (o tabuleiro físico, as peças).
